...
## 🧩 Canonical TypeInfo schema & Registry API (practical reference)

The sections above are conceptual. Below is a small, copy-pasteable reference that can be used when implementing `DataRegistry` and the first `IBindingGenerator` prototype.

### Canonical reflection payload (TypeInfo)

````cpp
struct FunctionInfo {
    std::string name;                    // exposed name, e.g. "click"
    std::string cpp_signature;           // human-readable C++ signature
    std::vector<std::string> param_types; // canonical SDOM type names
    std::string return_type;             // canonical SDOM type name
    std::string c_name;                  // optional explicit C symbol name
    std::string c_signature;             // optional explicit C prototype
    std::string doc;                     // short help text
    bool is_static = false;
    bool exported = true;                // visible to generators
};

struct PropertyInfo {
    std::string name;        // "text"
    std::string cpp_type;    // "std::string"
    bool read_only = false;
    std::optional<std::string> default_value; // serialized form
    std::string doc;
};

struct TypeInfo {
    std::string name; // "Button"
    std::string cpp_type_id; // optional unique id or typeid(T).name()
    std::vector<PropertyInfo> properties;
    std::vector<FunctionInfo> functions;
    std::vector<std::string> bases; // base type names
    std::string doc;
};
````

### Registry API sketch (thread-safe, idempotent)

````cpp
class DataRegistry {
public:
    // Type-level registration
    bool registerType(const TypeInfo& info); // returns false on duplicate/invalid

    // Member registration (convenience templates forward metadata + callables)
    template<typename Fn>
    void registerFunction(const std::string& typeName, const FunctionInfo& meta, Fn&& callable);
    ...
    // Querying
    const TypeInfo* lookupType(const std::string& name) const; // nullptr if missing
    std::vector<std::string> listTypes() const;

    // Binding generation
    bool generateBindings(const std::string& outputDir); // invokes registered generators
};
````

Notes:
- Registration is idempotent: multiple calls for the same metadata should be safe.
- The runtime should keep a minimal callable wrapper (see design note below) so generators can emit thin thunks that call into C++.

---

## 🛠 Generated C header conventions (guidelines)

- Header must include a version macro and generation timestamp:

```cpp
/* Generated by SDOM DataRegistry */
#define SDOM_CAPI_VERSION "2025-11-12"
```

- Opaque handles use `typedef struct SDOM_<Type> SDOM_<Type>;` and are never exposed directly.
- Naming: default C symbol is `SDOM_<Type>_<member>`; generators may honor `FunctionInfo::c_name` when present.
- Ownership: creators emit paired `SDOM_<Type>* SDOM_Create_<Type>(...);` and `void SDOM_Destroy_<Type>(SDOM_<Type>*);` with explicit ownership semantics documented in the header.
- Strings: map `std::string` -> `const char*` in prototypes and document lifetime (engine-owned vs caller-owned). Provide helpers `SDOM_strdup` / `SDOM_free` if needed.
- Overloads: do NOT implicitly map overloaded C++ functions to the same C symbol. Require either:
  - explicit `c_name` metadata, or
  - automatic disambiguation via suffixes (_v1, _v2...) based on parameter shapes.

Example generated snippet (expanded):

```c
/* sdom_capi_objects.h — generated */
#ifndef SDOM_CAPI_OBJECTS_H
#define SDOM_CAPI_OBJECTS_H
#ifdef __cplusplus
extern "C" {
...
SDOM_Button* SDOM_Create_Button(void);
void SDOM_Destroy_Button(SDOM_Button*);

// property accessors
const char* SDOM_Button_getText(const SDOM_Button*);
void SDOM_Button_setText(SDOM_Button*, const char* text);
...
## 🔧 Type mapping, ABI & Versioning

- Provide a per-generator type map (CTypeAdapter) mapping canonical SDOM types to target types. Example: `std::string -> const char*`, `Variant -> void*`.
- Embed an `SDOM_CAPI_VERSION` macro and a generator checksum/hash into each header to detect drift between generated headers and runtime.
- For ABI stability, prefer simple types in the public API (integers, pointers, C-strings) and keep complex types behind `void*`/opaque handles.

---

## ✅ Testing & CI checklist for generator

1. Unit test that registers a small set of `TypeInfo` entries and asserts `generateBindings` writes files with expected symbols.
2. Compile test: call `gcc -c` on each generated header in CI to ensure syntactic validity.
3. ABI smoke test: compile a tiny C program that links to the SDOM static lib (if available) and calls one generated function.
4. Roundtrip hash test: compute a hash over reflection metadata and ensure regenerated headers change only when metadata changes.

---

## 🔁 Implementation note (callable wrappers)

Start metadata-first (store callables as `std::any`) and iterate to a small uniform invoker type once the Variant/Lua calling convention is settled. A recommended intermediate: `std::function<Variant(const VariantVector&)>` — this lets generators create thin C thunks that translate C types -> Variant call -> Variant -> C result.

---

## 📌 Next steps (docs -> code transition)

1. Land these doc edits as the canonical reference.
2. Implement `DataRegistry` scaffolding (metadata storage + generator registration) and the Phase‑1 `BindingGenerator` prototype that emits a compact `sdom_capi_objects.h` for a few core types.
3. Add the CI compile test to ensure generated headers remain valid.
