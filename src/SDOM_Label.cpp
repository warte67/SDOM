...
if (LABEL_DEBUG)
{
    ...
    void Label::_registerLuaBindings(const std::string& typeName, sol::state_view lua)
    {
        ...
        // To Be Deprecated: DisplayHandle::ensure_type_bind_table(lua, typeName, SUPER::TypeName); is the new 
        // standard for all display objects including assets
        ...
        if (absent("getLastTokenizedText")) {
            per_type_table.set_function("getLastTokenizedText", [this](DisplayHandle& self) -> std::string {
                return this->getLastTokenizedText();
            });
        }

        // Expose tokenizeText() so tests or scripts can force re-tokenization and get the token count
        if (absent("tokenizeText")) {
            per_type_table.set_function("tokenizeText", [this](DisplayHandle& self) -> int {
                return this->tokenizeText();
            });
        }

        if (absent("getTokenAlignLists")) {
            per_type_table.set_function("getTokenAlignLists", [this](DisplayHandle& self) -> sol::table {
                sol::state_view sv = SDOM::Core::getInstance().getLua();
                sol::table out = sv.create_table();
                const auto& m = this->getTokenAlignLists();
                int outer = 1;
        ...
        if (absent("getPhraseAlignLists")) {
            per_type_table.set_function("getPhraseAlignLists", [this](DisplayHandle& self) -> sol::table {
                sol::state_view sv = SDOM::Core::getInstance().getLua();
                sol::table out = sv.create_table();
                const auto& m = this->getPhraseAlignLists();
                int outer = 1;
                for (const auto& [align, vec] : m) {
                    sol::table inner = sv.create_table();
                    int i = 1;
        ...
        // Boolean flags (use helper to reduce duplication)
        {
            auto addBoolProp = [&](const char* name, auto getter, auto setter) {
                if (absent(name)) {
                    per_type_table.set(name, sol::property(
        ...
        // --- Also provide function-style getters/setters for legacy tests --- //
        {
            auto addBoolFuncs = [&](const char* setName, const char* getName, auto setter, auto getter) {
                if (absent(setName)) {
                    per_type_table.set_function(setName, [setter](DisplayHandle h, bool v){ auto* l = h.as<Label>(); if (l) (l->*setter)(v); });
                }
                if (absent(getName)) {
                    per_type_table.set_function(getName, [getter](DisplayHandle h)->bool{ auto* l = h.as<Label>(); return l ? (l->*getter)() : false; });
                }
            };
        ...
        // --- Also provide function-style getters/setters for integer legacy tests --- //
        {
            auto addIntFuncs = [&](const char* setName, const char* getName, auto setter, auto getter) {
                ...
                sol::object curSet = per_type_table.raw_get_or(setName, sol::lua_nil);
                if (!curSet.valid() || curSet == sol::lua_nil) {
                    per_type_table.set_function(setName, [setter](DisplayHandle h, int v){ auto* l = h.as<Label>(); if (l) (l->*setter)(v); });
                }
                // register getter if not already present
                sol::object curGet = per_type_table.raw_get_or(getName, sol::lua_nil);
                if (!curGet.valid() || curGet == sol::lua_nil) {
                    per_type_table.set_function(getName, [getter](DisplayHandle h)->int{ auto* l = h.as<Label>(); return l ? (l->*getter)() : 0; });
                }
            };
            ...
            per_type_table.set_function("setFontSize", [](DisplayHandle h, int v){ auto* l = h.as<Label>(); if (l) l->setFontSize(v); });
            per_type_table.set_function("getFontSize", [](DisplayHandle h)->int{ auto* l = h.as<Label>(); return l ? l->getFontSize() : 0; });
        ...
        per_type_table.set_function("setFontWidth", [](DisplayHandle h, int v){ auto* l = h.as<Label>(); if (l) l->setFontWidth(v); });
        per_type_table.set_function("getFontWidth", [](DisplayHandle h)->int{ auto* l = h.as<Label>(); return l ? l->getFontWidth() : 0; });
        if (DEBUG_REGISTER_LUA) std::cout << "Registering Label::setFontHeight/getFontHeight override on DisplayHandle" << std::endl;
        per_type_table.set_function("setFontHeight", [](DisplayHandle h, int v){ auto* l = h.as<Label>(); if (l) l->setFontHeight(v); });
        per_type_table.set_function("getFontHeight", [](DisplayHandle h)->int{ auto* l = h.as<Label>(); return l ? l->getFontHeight() : 0; });

        // Alignment (enum) and alignment_string (string)
        if (absent("alignment")) {
            per_type_table.set("alignment", sol::property(
        ...
        if (absent("alignment_string")) {
            per_type_table.set("alignment_string", sol::property(
        ...
        sol::object cur_getAlign = per_type_table.raw_get_or("getAlignment", sol::lua_nil);
        if (!cur_getAlign.valid() || cur_getAlign == sol::lua_nil) {
            per_type_table.set_function("getAlignment", [](DisplayHandle h)->int { auto* l = h.as<Label>(); return l ? static_cast<int>(l->getAlignment()) : static_cast<int>(LabelAlign::TOP_LEFT); });
        }
        // getAlignmentString
        sol::object cur_getAlignStr = per_type_table.raw_get_or("getAlignmentString", sol::lua_nil);
        if (!cur_getAlignStr.valid() || cur_getAlignStr == sol::lua_nil) {
            per_type_table.set_function("getAlignmentString", [](DisplayHandle h)->std::string { auto* l = h.as<Label>(); return l ? l->getAlignmentString() : std::string("default"); });
        }
        // setAlignment: support both numeric enum and string overloads
        sol::object cur_setAlign = per_type_table.raw_get_or("setAlignment", sol::lua_nil);
        if (!cur_setAlign.valid() || cur_setAlign == sol::lua_nil) {
            per_type_table.set_function("setAlignment",
                sol::overload(
        ...
        if (absent("foreground_color")) {
            per_type_table.set("foreground_color", sol::property(
        ...
        if (absent("background_color")) {
            per_type_table.set("background_color", sol::property(
        ...
        if (absent("border_color")) {
            per_type_table.set("border_color", sol::property(
        ...
        if (absent("outline_color")) {
            per_type_table.set("outline_color", sol::property(
        ...
        if (absent("dropshadow_color")) {
            per_type_table.set("dropshadow_color", sol::property(
        ...
        if (absent("getForegroundColor")) {
            per_type_table.set_function("getForegroundColor", [=](DisplayHandle h) -> sol::object {
        ...
        if (absent("setForegroundColor")) {
            per_type_table.set_function("setForegroundColor", [=](DisplayHandle h, sol::table t) { Label* l = h.as<Label>(); if (!l) return; l->setForegroundColor(table_to_color(t)); });
        }
        if (absent("getBackgroundColor")) {
            per_type_table.set_function("getBackgroundColor", [=](DisplayHandle h) -> sol::object { Label* l = h.as<Label>(); sol::state_view sv = SDOM::Core::getInstance().getLua(); if (!l) return sol::nil; return color_to_table(sv, l->getBackgroundColor()); });
        }
        if (absent("setBackgroundColor")) {
            per_type_table.set_function("setBackgroundColor", [=](DisplayHandle h, sol::table t) { Label* l = h.as<Label>(); if (!l) return; l->setBackgroundColor(table_to_color(t)); });
        }
        if (absent("getBorderColor")) {
            per_type_table.set_function("getBorderColor", [=](DisplayHandle h) -> sol::object { Label* l = h.as<Label>(); sol::state_view sv = SDOM::Core::getInstance().getLua(); if (!l) return sol::nil; return color_to_table(sv, l->getBorderColor()); });
        }
        if (absent("setBorderColor")) {
            per_type_table.set_function("setBorderColor", [=](DisplayHandle h, sol::table t) { Label* l = h.as<Label>(); if (!l) return; l->setBorderColor(table_to_color(t)); });
        }
        if (absent("getOutlineColor")) {
            per_type_table.set_function("getOutlineColor", [=](DisplayHandle h) -> sol::object { Label* l = h.as<Label>(); sol::state_view sv = SDOM::Core::getInstance().getLua(); if (!l) return sol::nil; return color_to_table(sv, l->getOutlineColor()); });
        }
        if (absent("setOutlineColor")) {
            per_type_table.set_function("setOutlineColor", [=](DisplayHandle h, sol::table t) { Label* l = h.as<Label>(); if (!l) return; l->setOutlineColor(table_to_color(t)); });
        }
        if (absent("getDropshadowColor")) {
            per_type_table.set_function("getDropshadowColor", [=](DisplayHandle h) -> sol::object { Label* l = h.as<Label>(); sol::state_view sv = SDOM::Core::getInstance().getLua(); if (!l) return sol::nil; return color_to_table(sv, l->getDropshadowColor()); });
        }
        if (absent("setDropshadowColor")) {
            per_type_table.set_function("setDropshadowColor", [=](DisplayHandle h, sol::table t) { Label* l = h.as<Label>(); if (!l) return; l->setDropshadowColor(table_to_color(t)); });
        }
