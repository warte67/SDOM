...
namespace SDOM
{
    ...
    Checkbox::Checkbox(const sol::table& config) : IPanelObject(config)
    {
        ...
        if (config["is_checked"].valid()) {
            try { isChecked_ = config["is_checked"].get<bool>(); } catch(...) {}
        } else if (config["checked"].valid()) {
            try { isChecked_ = config["checked"].get<bool>(); } catch(...) {}
        } else if (config["is_selected"].valid()) {
            try { isChecked_ = config["is_selected"].get<bool>(); } catch(...) {}
        } else if (config["selected"].valid()) {
            try { isChecked_ = config["selected"].get<bool>(); } catch(...) {}
        } else {
            isChecked_ = init.isChecked;
        }
    ...
    void Checkbox::setChecked(bool checked)
    {
        ...
        stateChanged->setPayloadValue("previous_selected", previous);
        stateChanged->setPayloadValue("selected", isChecked_);
