...
namespace SDOM
{
    ...
    bool Stage_UnitTests()
    {
        ...
        testResult = UnitTests::run("Stage #1", "Retrieve 'mainStage'", [factory]() {
            DisplayHandle stageHandle = factory->getDisplayObject("mainStage");
        ...
        testResult = UnitTests::run("Stage #2", "Retrieve non-existent stage", [factory]() {
            DisplayHandle stageHandle = factory->getDisplayObject("nonExistentStage");
        ...
        //     DisplayHandle rootHandle = core.getRootNode();
        ...
        // Test: Set stage to "stageTwo" using setRootNode with DisplayHandle
        std::string newStage = "stageTwo";
        testResult = UnitTests::run("Stage #4", "Set root stage to '" + newStage + "' using DisplayHandle", [&core, factory, &dbgStr, newStage]() {
            DisplayHandle stageTwoHandle = factory->getDisplayObject(newStage);
